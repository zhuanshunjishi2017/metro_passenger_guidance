...
static int flag;
void twocolumns(lv_obj_t* canvas)
{
	lv_draw_rect_dsc_t column1;
	lv_draw_rect_dsc_init(&column1);
	column1.bg_opa = LV_OPA_COVER;
	column1.bg_color =lv_color_hex(0x1a51);
	lv_canvas_draw_rect(canvas,0,55,60,545,&column1);

	lv_draw_rect_dsc_t column2;
	lv_draw_rect_dsc_init(&column2);
	column1.bg_opa = LV_OPA_COVER;
	column1.bg_color =lv_color_hex(0xd6dc);
	lv_canvas_draw_rect(canvas,0,0,1024,55,&column2);
}
void create_buttons(lv_obj_t* canvas,lv_obj_t* btn1,lv_obj_t* btn2,lv_obj_t* btn3)
{
	btn1 = lv_btn_create(canvas);
	lv_obj_set_pos(btn1, 6, 70);
	lv_obj_set_size(btn1, 50, 66);
	lv_obj_set_style_radius(btn1,4,LV_PART_MAIN);
	lv_obj_set_style_bg_color(btn1,lv_color_hex(0xffff),LV_PART_MAIN);
	lv_obj_add_event_cb(btn1,btn1_cb,LV_EVENT_ALL,NULL);

	btn2 = lv_btn_create(canvas);
...
void creat_top_ta(lv_obj_t* canvas,lv_obj_t* ta,lv_obj_t* kb)
{
	ta = lv_textarea_create(canvas);
	lv_obj_set_pos(ta,264,12);
	lv_obj_set_size(ta,266,34);
	lv_obj_set_style_radius(ta,4,LV_PART_MAIN);
	kb = lv_keyboard_create(canvas);
...
void btn2_cb(lv_event_t *e)
{
	...
	{
		flag = 1;
		lv_obj_set_style_bg_color(btn2,lv_color_hex(0xffff),LV_PART_MAIN);
...
void btn3_cb(lv_event_t *e)
{
	...
	{
		flag = 2;
		lv_obj_set_style_bg_color(btn3,lv_color_hex(0xffff),LV_PART_MAIN);
