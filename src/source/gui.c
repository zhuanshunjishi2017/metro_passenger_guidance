...
extern void del_pp_window(void);
extern void favorite_station_lb_style_reset(void);

extern void search_result_show_label_set_text(const char * text);


extern void search_record_load_from_sd(void);
extern void search_record_save_to_sd(void);
    ...
    for (int i = 0; i < LINE_COUNT; i++)
    {
        for (int j = 0; j < metro_lines[i].count; j++)
        {
                ...
                && metro_lines[i].stations[j].transfer_line[0] >= 0) 
            {
                ...
                lv_label_set_text_fmt(top_search_line[index], "%d号线", metro_lines[i].line_number);
...
void tab_search_btn_cb(lv_event_t *e)
{
	...
	int done = 0;
	for (int i = 0; i < LINE_COUNT && !done; i++)
    {
        for (int j = 0; j < metro_lines[i].count; j++)
        {
